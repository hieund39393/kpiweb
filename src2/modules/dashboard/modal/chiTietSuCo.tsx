import { Button, Modal, Row, Col, Form, Pagination, DatePicker } from 'antd';
...
function ChiTietSuCo({ ngayGanNhat, ngaySuCo, totalSuCo, detail, chiTieuID, chiTieuEVNID, donViID, loaiSuCo, tenSuCo, ngayBaoCao, isShowModal, closeModal }) {
  ...
  const changeDate = async (date: any) => {
    ...
    const convert = ngay.year() + '-' + formatDate(ngay.month() + 1) + '-' + formatDate(ngay.date())
    setDate(convert)
    return await fetchNgaySuCo(chiTieuID, donViID, convert, _PAGEINDEX, _PAGESIZE)
  }

  //lấy các sự cố theo ngày
  async function fetchNgaySuCo(chiTieuID, donViID, convert, pageIndex, pageSize) {
    const response = await chartService.listDetail({ chiTieuID: chiTieuID, chiTieuEVNID: chiTieuEVNID, donViID: donViID, ngay: convert, layNgayGanNhat: _FALSE, pageIndex: pageIndex, pageSize: pageSize })
  ...
  function convertNgay(ngay) {
    if (ngay && ngay.length) {
      ...
      ngay.map(item => {
        ...
        const value = ngay.year() + '-' + formatDate((ngay.month() + 1)) + '-' + formatDate(ngay.date())
        const convert = formatDate(ngay.date()) + '/' + formatDate((ngay.month() + 1)) + '/' + ngay.year()
        arr.push({
          value: value,
          ngay: convert
        })
  ...
  return (
    ...
    >
      ...
      <div className="modal-detail">
        ...
        {total > 20 ?
          <Pagination defaultCurrent={1} total={total} defaultPageSize={20} showSizeChanger onChange={onChange} onShowSizeChange={onShowSizeChange} />
          : ''}
      </div>
    </Modal>
  )
