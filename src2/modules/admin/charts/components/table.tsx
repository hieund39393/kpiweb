...
import { BoChiTieuProps, ListChartProps, LoaiChiTieuLv3Props } from "../dtos/responses/ListChartResponse"
...
function DanhSachBieuDo() {
  ...
  async function fetchBieuDo(groupID: string, chiTieuLV3: string) {
    ...
    let response = await listChartService.list({ data: data, pageIndex: _PAGEINDEX, pageSize: _PAGESIZE })
    if (response.data) {
  ...
  const columns = [
    {
      ...
      key: "tenBieuDo",
      width: '20%',
      sorter: data && data.length > 0 ? (a, b) => { return a.tenBieuDo.length - b.tenBieuDo.length } : false,
    },
    {
      ...
      key: "summary",
      width: '26%',
      render: (text: null) => text === 1 || text === '1' ? 'Hiển thị thông tin summary' : text === 2 || text === '2' ? 'Không hiển thị summary' : 'Không cho phép chỉnh sửa biểu đồ'
    },
    {
  ...
  async function filterHandler(values: any) {
    if (values.timKiem === undefined || values.timKiem === '') return fetchBieuDo(groupID, loaiChiTieu);
    ...
    let response = await listChartService.search({ data: newData, pageIndex: _PAGEINDEX, pageSize: _PAGESIZE })
    if (response.statusCode === _STATUSCODE200) {
      ...
      <>
        ...
        >
          <Row gutter={16} className="box-filter form-filter-chart">
            ...
            <Col span={12} md={3} className="button-filter-chart form-filter-chart--reset filter-pc">
              <Button className="button-closed " onClick={onReset}>Bỏ lọc</Button>
            </Col>
            <Col span={12} md={6} className="button-filter-chart form-filter-chart--submit">
              <Button className="button-closed filter-mobile" onClick={onReset}>Bỏ lọc</Button>
              {/* <Button className="button-primary button-create-auto" onClick={() => createAuto()} type="default" >Tạo tự động</Button> */}
              <Button className="button-primary button-submit-chart" htmlType="submit" type="default" >Lấy dữ liệu</Button>
