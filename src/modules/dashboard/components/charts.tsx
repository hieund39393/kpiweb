...
function Charts(props) {
  const { donViID, ngayBaoCao, fetchAt, indicatorsLevel, boChiTieuDisplayID, setIsChange, isChange } = props;
  ...
  return (
    <div className="bct-data__charts">
      {dataChartLevel2.map((element: { title, charts, isRow, periodData, isTable, ctid, group }, index) => {

        if (element?.group === 1) {
          return (
            <div className="page-setOfIndicators--content content-body" key={index}>
              <div className="content-title title-lv2">
                <h2 id={element?.ctid} style={{ textAlign: 'center' }}>{element?.title}</h2>
          ...
          if (element?.ctid === 'CT_23' && element?.charts[0]?.charts.length === 6) classTrungThe = 'sixLength'
          else if (element?.ctid === 'CT_23' && element?.charts[0]?.charts.length >= 8) classTrungThe = 'eightLength'
          else classTrungThe = ''
          return (
            element !== undefined && (
              // <div key={index}>
              <Periods
                ...
                isChange={isChange} classTrungThe={classTrungThe} />
            )
          );
        }
      })}
